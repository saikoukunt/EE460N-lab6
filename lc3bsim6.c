...
void SR_stage() {
  ...
  sr_reg_data = Low16bits(sr_reg_data);
...
int v_dcache_en, dcache_addr, read_word, write_word, dcache_r;
int mem_w0, mem_w1;

int target_pc, trap_pc, mem_pcmux;
...
void MEM_stage() {
  ...
  //dcache access
  dcache_addr = PS.MEM_ADDRESS;
  write_word = (Get_DATA_SIZE(PS.MEM_CS) && (PS.MEM_ADDRESS % 2)) ? (PS.MEM_ALU_RESULT & 0xFF) * 0x0101: PS.MEM_ALU_RESULT;
  ...
  if(PS.MEM_V){
    if(Get_UNCOND_OP(PS.MEM_CS)) mem_pcmux = 1;
      ...
      int BEN = ((PS.MEM_CC & 0x4) && (PS.MEM_IR & 0x800)) || ((PS.MEM_CC & 0x2) && (PS.MEM_IR & 0x400)) ||
...
void AGEX_stage() {
  ...
  //SHF
  int shfOut = PS.AGEX_SR1;
  if(PS.AGEX_IR & 0x0010){
    if(PS.AGEX_IR & 0x0020){
      int sign = PS.AGEX_SR1 >> 15;
      for(int i = 0; i < (PS.AGEX_IR & 0x000F); i++){
        shfOut = (sign << 15) + (shfOut>> 1);
      }
    }
    else shfOut = shfOut >> (PS.AGEX_IR & 0x000F); 
  }
  else shfOut = shfOut << (PS.AGEX_IR & 0x000F);
  ...
  if (LD_MEM) {
    ...
    NEW_PS.MEM_ALU_RESULT = resultOut & 0xFFFF;
...
void DE_stage() {
  ...
  //stall
  v_de_br_stall = cStore[BR_STALL] && PS.DE_V;

  //source register ids
  int sr1_id = (PS.DE_IR >> 6) & 7;
  int sr2_id = (PS.DE_IR & 0x2000) ? ((PS.DE_IR >> 9) & 0x7) : PS.DE_IR & 0x7;
  
  //dependency check
  dep_stall = 0;
  if(PS.DE_V){
    if(cStore[BR_OP] & (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc)) dep_stall = 1;
  }
  if(v_agex_ld_reg){
    if(((sr1_id == PS.AGEX_DRID) && cStore[SR1_NEEDED]) ||
        ((sr2_id == PS.AGEX_DRID) && cStore[SR2_NEEDED]))
        dep_stall = 1;
  }
  if(v_mem_ld_reg){
    if(((sr1_id == PS.MEM_DRID) && cStore[SR1_NEEDED]) ||
        ((sr2_id == PS.MEM_DRID) && cStore[SR2_NEEDED]))
        dep_stall = 1;
  }
  if(v_sr_ld_reg){
    if(((sr1_id == PS.SR_DRID) && cStore[SR1_NEEDED]) ||
        ((sr2_id == PS.SR_DRID) && cStore[SR2_NEEDED]))
        dep_stall = 1;
  }
...
void FETCH_stage() {
  ...
  //calculate LD.PC, LD.DE
  int ld_pc = (icache_r && !(dep_stall || mem_stall || v_de_br_stall ||
        v_agex_br_stall || v_mem_br_stall)) || mem_pcmux; //any kind of stall
  int ld_de = !(dep_stall || mem_stall); //de_v will take care of branch stalls
